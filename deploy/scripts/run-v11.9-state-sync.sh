...
# provider still performs the real signed auto-advance ceremony through app-v28,
...
# 1. Drive a fresh real chain through the signed app-v28 ladder. The scoped
# projection installed below still proves app-v20's governance-domain semantics.
write_provider_personal_config
cat >"${PROVIDER_HOME}/post-v28.txt" <<'EOF'
This committed post-app-v28 fixture record proves the provider snapshot is beyond the positive activation height.
EOF
...
EOF
chmod 0644 "${PROVIDER_HOME}/post-v28.txt" "${PROVIDER_HOME}/advance.txt"
...
if [ "${snapshot_height}" -le 1 ]; then
  echo "ERROR: provider did not reach a positive post-app-v28 snapshot height" >&2
  exit 1
fi
...
if [ "${receiver_app_version}" != "${TARGET_APP_VERSION}" ] ||
  ...
  echo "ERROR: restarted receiver/provider did not remain on and converge at exact app-v28 state" >&2
  exit 1
fi
...
if [ "${success_receiver_app_version}" != "${TARGET_APP_VERSION}" ] ||
  ...
  echo "ERROR: successful receiver/provider did not publish exact app-v28 state" >&2
  exit 1
fi
...
if [ "${post_restart_provider_version}" != "${TARGET_APP_VERSION}" ] ||
  ...
  echo "ERROR: provider SIGKILL recovery regressed exact app-v28 state (${post_restart_provider_version:-unknown}/${post_restart_receiver_version:-unknown})" >&2
  exit 1
fi
