...
// fakeHunchServer answers the write gate's checks from keywords in the
...
func fakeHunchServer(t *testing.T) *httptest.Server {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for id := range req.Checks {
			...
			case "replaces":
				if strings.Contains(req.Context["new"], "now opens at 08:00") &&
					strings.Contains(req.Context["old"], "opens at 07:00") {
					p = 0.97
				}
			case "agrees":
				p = 0.02
...
// gateMemoryRaw builds a signed memory submission with a caller-chosen ID, so
// the test can stage the embedding exactly as the REST handler does.
...
// TestWriteGateEndToEndOnARealNode boots a real single-validator node, runs the
// real voter with the write gate against a scripted judge, and checks every
// gate outcome on committed state: accept, reject, abstain-then-review, and a
// correction that hides what it corrects without deprecating it.
func TestWriteGateEndToEndOnARealNode(t *testing.T) {
	...
	go voter.Run(ctx, app, projection, voter.Config{
		...
		Gate: &voter.Gate{Judges: []voter.Judger{hunch.New(judge.URL, "", "", 10*time.Second)}, Neighbours: 5},
	}, zerolog.Nop())

	agentKey := agentKeyFromBootstrap(t, bootstrap)
	nonce := uint64(0)
	submit := func(id, content string, emb []float32) {
		t.Helper()
		nonce++
		app.SuppCache.Put(id, &memory.SupplementaryData{Embedding: emb, EmbeddingProvider: "test"})
	...
	submit(oldID, "The depot opens at 07:00 on weekdays.", []float32{1, 0, 0})
	waitStatus(oldID, committed, "a lasting fact is accepted")

	// 2. A remark about the agent's own session is voted down.
	remarkID := "00000000-0000-4000-8000-000000000002"
	submit(remarkID, "The attachment was lost; the user must re-send the ten numbers.", []float32{0, 1, 0})
	...
	submit(unsureID, "The quarterly shipment count was around two hundred units.", []float32{0, 0, 1})
	...
	// 4. A correction is committed and MARKS the old fact superseded; the old
	// fact itself stays committed (hidden from default recall, not deprecated).
	newID := "00000000-0000-4000-8000-000000000004"
	submit(newID, "The depot now opens at 08:00 on weekdays.", []float32{0.99, 0.01, 0})
	waitStatus(newID, committed, "the correction is accepted")
	require.Eventually(t, func() bool {
		ann, err := projection.GateAnnotationsFor(ctx, []string{oldID})
		return err == nil && ann.SupersededBy[oldID] == newID
	}, 45*time.Second, 200*time.Millisecond, "the old fact is marked superseded by the committed correction")
	require.Equal(t, memory.StatusCommitted, statusOf(oldID), "marking never deprecates")
